...
public class AdminWeb { // ESTO SEGURO SE PUEDE MEJORAR MAS ADELANTE.
	// L: Para mi hay que eliminar esta clase y colocar los metodos de dar de alta
	// en sitio
	...
	public List<Categoria> getListaDeCategoriasValidas(TipoRankeable tipoRankeable) {
		return categorias.stream().filter(categoria -> categoria.getTipoRankeable() == (tipoRankeable)).toList();
	}

	public boolean esCategoriaValida(Categoria categoria) {
		return getListaDeCategoriasValidas(categoria.getTipoRankeable()).contains(categoria);
	}
	...
	public List<Servicio> getServiciosInmuebles() {
		return serviciosInmuebles;
	}

	public void setServiciosInmuebles(List<Servicio> serviciosInmuebles) {
	...
	public void setTiposDeInmueble(List<TipoInmueble> tiposDeInmueble) {
	...
	/*
	 * public void validarRanking(Usuario u, String comentario, int puntaje, String
	 * categoria) { if (categorias.contains(categoria)) { RankingUsuario r = new
	 * RankingUsuario(comentario, puntaje, categoria, u); // DAR DE ALTA
	 * rankings.add(r); } else { throw new
	 * CategoriaException("La categoria no está registrada en el sistema."); } }
	 * 
	 * public void validarRankingDeInmueble(Inmueble i, String comentario, int
	 * puntaje, String categoria) { if (categorias.contains(categoria)) {
	 * RankingInmueble r = new RankingInmueble(categoria, puntaje, comentario, i);
	 * // DAR DE ALTA rankings.add(r); } else { throw new
	 * IllegalArgumentException("La categoria no está registrada en el sistema."); }
	 * }
	 */
}
